...
import os
import qrcode  # Если не установлено, добавьте в environment или используйте text-token
...
def get_user_roles(user_id):
    ...
    result = cur.fetchone()
    return json.loads(result["roles"]) if result else []


# Helper to get user_id from telegram_id (используйте вашу функцию из кода)
def get_user_id_from_telegram(telegram_id):
    db = get_db()
    cur = db.cursor()
    cur.execute(
        "SELECT user_id FROM telegram_links WHERE telegram_id = ?", (str(telegram_id),)
    )
    result = cur.fetchone()
    return result["user_id"] if result else None


# /start - Первое взаимодействие (раздел 13.1)
def start(update, context):
    ...
    if not user_id:
        update.message.reply_text(
            "Привет! Чтобы использовать бота, свяжи аккаунт в KipCalendar. Введи код: /link <код>"
        )
    else:
        update.message.reply_text(
            "Добро пожаловать обратно! Используй /help для списка команд."
        )
...
# /link - Связывание аккаунта (раздел 2.2)
def link(update, context):
    if len(context.args) != 1:
        update.message.reply_text("Использование: /link <6-значный код>")
        return

    code = context.args[0]
    telegram_id = update.message.from_user.id

    # Вызов вашей функции link_telegram (из существующего кода)
    # Предполагаю, что у вас есть функция или эндпоинт; адаптируйте
    db = get_db()
    cur = db.cursor()
    cur.execute(
        "SELECT user_id FROM telegram_links WHERE link_code = ? AND link_expires > ?",
        (code, int(time.time())),
    )
    result = cur.fetchone()
    ...
    if result:
        ...
        cur.execute(
            "UPDATE telegram_links SET telegram_id = ?, link_code = NULL, link_expires = NULL WHERE user_id = ?",
            (str(telegram_id), user_id),
        )
        db.commit()
        update.message.reply_text("Аккаунт успешно связан!")
        create_notification(user_id, "system", "Telegram аккаунт связан")
    else:
        update.message.reply_text("Неверный или истекший код.")


application.add_handler(CommandHandler("link", link))


# /schedule - Расписание (раздел 5.1.1)
def schedule(update, context):
    telegram_id = update.message.from_user.id
    user_id = get_user_id_from_telegram(telegram_id)
    if not user_id:
        update.message.reply_text("Сначала свяжи аккаунт: /link <код>")
        return

    param = " ".join(context.args).lower() if context.args else "today"

    db = get_db()
    cur = db.cursor()
    # Пример запроса расписания (адаптируйте под вашу БД)
    if param == "today":
        cur.execute(
            """
            SELECT l.start_time, l.end_time, s.name FROM lessons l
        ...
        if lessons:
            text = "Расписание на сегодня:\n" + "\n".join(
                [
                    f"{row['start_time']}-{row['end_time']}: {row['name']}"
                    for row in lessons
                ]
            )
        else:
...
# Добавьте аналогично для 'tomorrow', 'week', '[date]'

application.add_handler(CommandHandler("schedule", schedule))


# /homework - Домашнее задание (раздел 5.1.2)
def homework(update, context):
    telegram_id = update.message.from_user.id
    user_id = get_user_id_from_telegram(telegram_id)
    if not user_id:
        update.message.reply_text("Сначала свяжи аккаунт.")
        return
    ...
    cur.execute(
        """
        SELECT al.date, s.name, al.homework FROM actual_lessons al
        JOIN lessons l ON al.lesson_id = l.id
        JOIN subjects s ON l.subject_id = s.id
        JOIN user_groups ug ON l.group_id = ug.group_id
        WHERE ug.user_id = ? AND al.homework IS NOT NULL AND al.date >= date('now')
        ORDER BY al.date LIMIT 5
    """,
        (user_id,),
    )
    tasks = cur.fetchall()
    ...
    text = (
        "Текущие ДЗ:\n"
        + "\n".join(
            [f"{row['date']} {row['name']}: {row['homework']}" for row in tasks]
        )
        if tasks
        else "Нет ДЗ."
    )
...
# /grades - Оценки (раздел 5.1.3)
def grades(update, context):
    # Аналогично homework, запросите marks из БД и отправьте
    pass  # Реализуйте по аналогии


application.add_handler(CommandHandler("grades", grades))


# /attendance - Посещаемость (раздел 5.1.4)
def attendance(update, context):
    # Запросите attendance из БД
    pass
...
# /calendar - Календарь (раздел 5.2.1)
...
# /profile - Профиль (раздел 5.2.2)
def profile(update, context):
    # Запросите users и organization_members
    pass
...
# /help - Помощь (раздел 5.3.1)
def help_cmd(update, context):
    text = "Доступные команды:\n/schedule - Расписание\n/homework - ДЗ\n/grades - Оценки\n..."  # Полный список из документа
    update.message.reply_text(text)
...
# /settings - Настройки (раздел 5.3.2)
def settings(update, context):
    # Inline-кнопки для управления уведомлениями (используйте InlineKeyboardMarkup)
    keyboard = [
        [InlineKeyboardButton("Вкл/Выкл уведомления", callback_data="toggle_notif")]
    ]
...
# Callback для inline-кнопок (раздел 8.2)
def button_callback(update, context):
    query = update.callback_query
    if query.data == "toggle_notif":
        # Обновите telegram_notification_settings в БД
        query.answer("Уведомления переключены!")


application.add_handler(CallbackQueryHandler(button_callback))


# Команды для преподавателей (раздел 6)
...
# Аналогично для /attendance_mark, /homework_assign, /qr_attendance, etc.


# Команды для админов (раздел 7)
def admin_stats(update, context):
    # Вызовите вашу функцию get_telegram_admin_stats
    pass


application.add_handler(CommandHandler("admin_stats", admin_stats))

# ... Добавьте остальные по аналогии (feedback, search, export, language)


# /feedback - Обратная связь (раздел 5.3.3)
def feedback(update, context):
    ...
    if not context.args:
        update.message.reply_text("Использование: /feedback <ваше сообщение>")
        return

    message = " ".join(context.args)

    # Сохраняем фидбек (например, в notifications для админа или отдельной таблице)
    db = get_db()
    cur = db.cursor()
    feedback_id = generate_uuid()
    cur.execute(
        """INSERT INTO notifications (id, user_id, type, content, timestamp, is_read)
           VALUES (?, ?, ?, ?, ?, 0)""",
        (
            feedback_id,
            "ADMIN_USER_ID",
            "feedback",
            f"Feedback from {user_id}: {message}",
            int(time.time()),
        ),  # Замените 'ADMIN_USER_ID' на реальный ID админа
    )
    db.commit()
    ...
    log_audit(user_id, "FEEDBACK_SENT", feedback_id)


application.add_handler(CommandHandler("feedback", feedback))


# /search - Поиск (раздел 8.3)
def search(update, context):
    ...
    if not context.args:
        update.message.reply_text(
            "Использование: /search <запрос> [тип: student/teacher/subject/room]"
        )
        return

    query = (
        " ".join(context.args[:-1]) if len(context.args) > 1 else " ".join(context.args)
    )
    search_type = (
        context.args[-1]
        if len(context.args) > 1
        and context.args[-1] in ["student", "teacher", "subject", "room"]
        else "all"
    )

    # Вызов вашего эндпоинта /api/telegram/search (адаптируйте)
    # Здесь симулируем запрос к БД; в реальности используйте requests.get к вашему API
    db = get_db()
    cur = db.cursor()
    ...
    if search_type in ["all", "student"]:
        cur.execute(
            """SELECT first_name, last_name FROM users 
               WHERE roles LIKE '%student%' AND (first_name LIKE ? OR last_name LIKE ?) LIMIT 5""",
            (f"%{query}%", f"%{query}%"),
        )
        results.extend(
            [
                f"Студент: {row['first_name']} {row['last_name']}"
                for row in cur.fetchall()
            ]
        )

    # Аналогично для teacher, subject, room (добавьте запросы)
...
# /export - Экспорт данных (раздел 8.4)
def export(update, context):
    ...
    param = " ".join(context.args).lower() if context.args else "schedule"

    db = get_db()
    cur = db.cursor()

    if param == "schedule":
        # Экспорт расписания в iCal (симулируем)
        cur.execute(
            """SELECT l.day_of_week, l.start_time, s.name FROM lessons l
               JOIN subjects s ON l.subject_id = s.id
               JOIN user_groups ug ON l.group_id = ug.group_id
               WHERE ug.user_id = ?""",
            (user_id,),
        ...
        data = cur.fetchall()
        # Создайте iCal строку или файл и отправьте
        ical_content = (
            "BEGIN:VCALENDAR\n"
            + "".join(
                [
        ...
        context.bot.send_document(
            ...
            document=ical_content.encode(),
            filename="schedule.ics",
        )

    # Аналогично для grades (CSV), attendance
    elif param == "grades":
        # Используйте pandas для CSV
        import pandas as pd

        cur.execute("SELECT * FROM marks WHERE student_id = ?", (user_id,))
    ...
    update.message.reply_text("Данные экспортированы.")


application.add_handler(CommandHandler("export", export))


# /language - Переключение языка (раздел 11.2)
def language(update, context):
    ...
    if not context.args:
        # Показать текущий и доступные
        db = get_db()
        cur = db.cursor()
        cur.execute(
            "SELECT language FROM telegram_notification_settings WHERE user_id = ?",
            (user_id,),
        )
        current = cur.fetchone()["language"] if cur.fetchone() else "ru"
        update.message.reply_text(
            f"Текущий язык: {current}. Доступные: ru, en. Используй /language <ru/en>"
        )
        return

    new_lang = context.args[0].lower()
    if new_lang not in ["ru", "en"]:
        update.message.reply_text("Неверный язык. Доступные: ru, en.")
        return

    db = get_db()
    cur = db.cursor()
    cur.execute(
        "UPDATE telegram_notification_settings SET language = ? WHERE user_id = ?",
        (new_lang, user_id),
    )
    db.commit()

    update.message.reply_text(
        "Язык изменен!" if new_lang == "ru" else "Language changed!"
    )
...
def grades(update, context):
    ...
    param = " ".join(context.args).lower() if context.args else "recent"
    db = get_db()
    cur = db.cursor()

    if param == "recent":
        cur.execute(
            "SELECT value, comment, created_at FROM marks WHERE student_id = ? ORDER BY created_at DESC LIMIT 5",
            (user_id,),
        )
        ...
        text = (
            ...
            + "\n".join(
                [
                    f"{row['created_at']}: {row['value']} ({row['comment']})"
                    for row in grades_list
                ]
            )
            if grades_list
            else "Нет недавних оценок."
        )
    elif param.startswith("average"):
        # Рассчитать средний балл (адаптируйте запрос)
        cur.execute(
            "SELECT AVG(value) as avg FROM marks WHERE student_id = ?", (user_id,)
        )
        avg = cur.fetchone()["avg"]
        text = f"Средний балл: {avg:.2f}" if avg else "Нет оценок."
    # Добавьте для [subject]
    else:
        text = "Неверный параметр."
...
def attendance(update, context):
    ...
    cur = db.cursor()
    cur.execute(
        "SELECT status, COUNT(*) FROM attendance WHERE student_id = ? GROUP BY status",
        (user_id,),
    )
    stats = cur.fetchall()
    text = (
        "Статистика посещаемости:\n"
        + "\n".join([f"{row['status']}: {row['COUNT(*)']}" for row in stats])
        if stats
        else "Нет данных."
    )
    update.message.reply_text(text)
...
def calendar(update, context):
    ...
    # Пример для today
    cur.execute(
        "SELECT title, time FROM events WHERE owner_id = ? AND date = date('now')",
        (user_id,),
    )
    events = cur.fetchall()
    text = (
        "События на сегодня:\n"
        + "\n".join([f"{row['time']}: {row['title']}" for row in events])
        if events
        else "Нет событий."
    )
    update.message.reply_text(text)
    # Добавьте для других params
...
def profile(update, context):
    ...
    cur = db.cursor()
    cur.execute(
        "SELECT first_name, last_name, roles FROM users WHERE id = ?", (user_id,)
    )
    user_data = cur.fetchone()
    text = f"Профиль: {user_data['first_name']} {user_data['last_name']}\nРоли: {user_data['roles']}"
    update.message.reply_text(text)
...
def attendance_mark(update, context):
    telegram_id = update.message.from_user.id
    user_id = get_user_id_from_telegram(telegram_id)
    if not user_id or "teacher" not in get_user_roles(
        user_id
    ):  # Добавьте функцию get_user_roles
        update.message.reply_text("Доступно только преподавателям.")
        return

    # Inline-кнопки для групп (пример)
    keyboard = [[InlineKeyboardButton("Группа 1", callback_data="group_1_attend")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text("Выберите группу для отметки:", reply_markup=reply_markup)
    # В button_callback обработайте выбор и отметку


application.add_handler(CommandHandler("attendance_mark", attendance_mark))


def homework_assign(update, context):
    # Аналогично attendance_mark, с inline или multi-message
    pass  # Реализуйте пошагово, сохраните в БД


application.add_handler(CommandHandler("homework_assign", homework_assign))


def groups(update, context):
    telegram_id = update.message.from_user.id
    user_id = get_user_id_from_telegram(telegram_id)
    if not user_id or "teacher" not in get_user_roles(user_id):
        update.message.reply_text("Доступно только преподавателям.")
        return
    ...
    cur = db.cursor()
    cur.execute(
        "SELECT DISTINCT g.name FROM groups g JOIN group_subjects gs ON g.id = gs.group_id WHERE gs.teacher_id = ?",
        (user_id,),
    )
    group_list = cur.fetchall()
    text = (
        "Ваши группы:\n" + "\n".join([row["name"] for row in group_list])
        if group_list
        else "Нет групп."
    )
    update.message.reply_text(text)
...
def stats(update, context):
    # Реализуйте запросы к marks и attendance
    pass
...
def qr_attendance(update, context):
    telegram_id = update.message.from_user.id
    user_id = get_user_id_from_telegram(telegram_id)
    if not user_id or "teacher" not in get_user_roles(user_id):
        update.message.reply_text("Доступно только преподавателям.")
    ...
    # Генерация токена (используйте вашу функцию)
    token = generate_uuid()  # Или полный процесс из ТЗ
    # Сохраните в БД
    ...
    img.save("qr.png")  # Временный файл
    context.bot.send_photo(chat_id=update.message.chat_id, photo=open("qr.png", "rb"))


application.add_handler(CommandHandler("qr_attendance", qr_attendance))


def scan(update, context):
    if len(context.args) != 1:
        update.message.reply_text("Использование: /scan <токен>")
        return
    token = context.args[0]
    # Проверьте и отметьте (используйте эндпоинт /api/qr-attendance/verify)
    pass
...
def admin_users(update, context):
    # Только для админов; реализуйте список с inline для действий
    pass
...
def admin_broadcast(update, context):
    # Только админы; соберите сообщение и отправьте через bot.send_message всем
    pass
...
def get_telegram_id_from_user(user_id):
    db = get_db()
    cur = db.cursor()
    cur.execute(
...
def echo(update, context):
    ...
    if text.isdigit() and len(text) == 6:  # Предполагаем код связывания
        context.args = [text]
        link(update, context)  # Вызов link как команды
    else:
...
def initiate_telegram_link():
    ...
    cur.execute(
        "SELECT * FROM telegram_links WHERE user_id = ? AND is_active = 1", (user_id,)
    )
    if cur.fetchone():
    ...
    cur.execute(
        "DELETE FROM telegram_links WHERE user_id = ? AND is_active = 0", (user_id,)
    )
    ...
    return jsonify(
        {"link_code": link_code, "expires_at": expires_at, "expires_in": 600}
    )
...
def complete_telegram_link():
    ...
    if cur.fetchone():
        return (
            jsonify({"error": "Telegram account already linked to another user"}),
            400,
        )
    ...
    return jsonify({"success": True, "user_id": user_id, "telegram_id": telegram_id})
...
def unlink_telegram_account():
    ...
    if user_id:
        ...
        cur.execute(
            "UPDATE telegram_links SET is_active = 0 WHERE user_id = ?", (user_id,)
        )
        ...
        cur.execute(
            "SELECT user_id FROM telegram_links WHERE telegram_id = ?", (telegram_id,)
        )
        row = cur.fetchone()
    ...
    cur.execute(
        "UPDATE telegram_tokens SET is_active = 0 WHERE user_id = ?", (target_user_id,)
    )
...
def generate_telegram_auth_token():
    ...
    cur.execute(
        "UPDATE telegram_tokens SET is_active = 0 WHERE user_id = ?", (user_id,)
    )
    ...
    return jsonify({"token": token, "expires_at": expires_at, "user_id": user_id})
...
def get_telegram_user_grades(user_id):
    ...
    try:
        numeric_grades = [
            float(g["value"])
            for g in grades
            if g["value"] and g["value"].replace(".", "").isdigit()
        ]
        avg_grade = (
...
def get_pending_telegram_notifications():
    ...
    if (
        not settings_row
        or json.loads(settings_row.get("settings", "{}")).get("enabled", True) == False
    ):
...
def telegram_add_grade():
    ...
    create_notification(
        student_id,
        "grade",
        f"Новая оценка по {subject_name}: {value}"
    ...
    return jsonify(
        {"success": True, "mark_id": mark_id, "message": "Grade added successfully"}
    )
...
def telegram_mark_attendance():
    ...
    return jsonify(
        {"success": True, "marked": marked_count, "errors": errors if errors else None}
    )
...
def telegram_notification_settings():
    ...
    if request.method == "GET":
        cur.execute(
            "SELECT * FROM telegram_notification_settings WHERE user_id = ?", (user_id,)
        )
        ...
        settings_dict["notification_types"] = json.loads(
            settings_dict.get("notification_types", "[]")
        )
...
def get_telegram_admin_stats():
    ...
    cur.execute(
        "SELECT COUNT(DISTINCT user_id) as count FROM telegram_statistics WHERE timestamp > ?",
...
def get_telegram_admin_users():
    ...
    if search:
        query += (
            " AND (u.username LIKE ? OR u.email LIKE ? OR tl.telegram_username LIKE ?)"
        )
...
def telegram_search():
    ...
    search_type = request.args.get(
        "type", "all"
    )  # all, student, teacher, subject, room
    ...
    results = {"students": [], "teachers": [], "subjects": [], "rooms": []}
...
def manage_telegram_bot_config():
    ...
    if request.method == "GET":
        cur.execute(
            "SELECT * FROM telegram_bot_config ORDER BY created_at DESC LIMIT 1"
        )
        config = cur.fetchone()
        ...
        cur.execute(
            "SELECT id FROM telegram_bot_config ORDER BY created_at DESC LIMIT 1"
        )
        existing = cur.fetchone()
